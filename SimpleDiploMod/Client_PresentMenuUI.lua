		...
		if(Mod.PlayerGameData.Nachrichten ~=nil)then
			...
			while(Nachrichtensplit[num] ~= nil and Nachrichtensplit[num+1] ~= nil and Nachrichtensplit[num+2] ~= nil and Nachrichtensplit[num+3] ~= nil)do
				...
				if(Nachrichtensplit[num+1] == "2")then
					Nachricht = Nachricht .. "\n" .. toname(tonumber(Nachrichtensplit[num]),Game) .. " accepted the peace offer by " .. toname(tonumber(Nachrichtensplit[num+3]),Game) .. " until turn " .. Nachrichtensplit[num+2]+1;
				end
			...
			if(Anzahl< 1)then
				...
				for _,terr in pairs(Game.Map.Territories)do
					...
					if(terr.Name == territory.GetText())then
						...
						UI.Alert(Game.Us.ID .. " " .. "Buy Armies (" .. Anzahl .. ") for " .. terr.Name .. pay.terrID .. " " .. pay.Count);
					end
				end
			end
		end
