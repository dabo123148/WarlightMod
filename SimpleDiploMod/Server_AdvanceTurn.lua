		...
		if(result.IsAttack and game.ServerGame.LatestTurnStanding.Territories[order.To].OwnerPlayerID ~= WL.PlayerID.Neutral)then
			if(InWar(order.PlayerID,game.ServerGame.LatestTurnStanding.Territories[order.To].OwnerPlayerID) == true)then
				if(result.IsSuccessful)then
					...
					error("Test" .. playerGameData[order.PlayerID].Money+result.AttackingArmiesKilled.NumArmies*Mod.Settings.MoneyPerKilledArmy);
					playerGameData[order.PlayerID].Money = playerGameData[order.PlayerID].Money + result.AttackingArmiesKilled.NumArmies*Mod.Settings.MoneyPerKilledArmy;
