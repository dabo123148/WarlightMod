...
function Client_PresentSettingsUI(rootParent)
	...
	if(Mod.Settings.NukeCardIn ~= nil and Mod.Settings.NukeCardIn)then
		...
		horz = UI.CreateHorizontalLayoutGroup(rootParent);
		UI.CreateLabel(horz).SetText('Territories hit by a nuke take ' .. (Mod.Settings.NukeCardMainTerritoryDamage) .. '% Damage');
		horz = UI.CreateHorizontalLayoutGroup(rootParent);
		UI.CreateLabel(horz).SetText('Connected Territories to a nuke take ' .. (Mod.Settings.NukeCardConnectedTerritoryDamage) .. '% Damage');
		horz = UI.CreateHorizontalLayoutGroup(rootParent);
		UI.CreateLabel(horz).SetText('Players can harm themselves: ' .. booltostring(Mod.Settings.Friendlyfire));
		horz = UI.CreateHorizontalLayoutGroup(rootParent);
		if(Mod.Settings.AfterDeployment)then
			UI.CreateLabel(horz).SetText('Territories get nuked AFTER Deployment but before Gift and Blockade Cards take effect');
		else
			UI.CreateLabel(horz).SetText('Territories get nuked BEFORE Deployment');
		end
		horz = UI.CreateHorizontalLayoutGroup(rootParent);
